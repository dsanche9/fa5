if [ $# != 6 ]
  ...
  echo "This script needs 5 arguments/variables to run; ELB-NAME, KEYPAIR, CLIENT-TOKENS,
   NUMBER OF INSTANCES, and SECURITY-GROUP-NAME & IAM-ROLE"
else
...
aws ec2 run-instances --image-id ami-8bb8c0bb --count $4 --instance-type t1.micro --iam-instance-profile Name=$6 --region us-west-2 --key-name $2 --security-group-ids $GROUPID --block-device-mappings "[{\"DeviceName\": \"/dev/sdh\",\"Ebs\":{\"VolumeSize\":100}}]" --subnet-id $SNID  --client-token $3 --user-data file://setup-MA5.sh --output=text
